...
<head>
    ...
    <title>Smart cab | Ola cab Rides</title>
...
<body>
    ...
    <div id="wrapper">
        ...
        <?php 
            include("nav.php"); 
            include("process/config.php");
            $rides = "SELECT r.*, cp.`name` AS `cpname`, c.`name` as `cname`, SUBSTRING_INDEX(u.`email`, '@', 1) AS `uname` FROM `rides` r, `fare` f, `company` cp, `car` c, `user` u WHERE r.`fid`=f.`id` AND r.`uid`=u.`id` AND f.`cid`=cp.`id` AND cp.`name`='Ola' AND f.`tcid`=c.`id` ORDER BY `id` DESC";
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12" style="margin-top:-30px">
                    <h1 class="page-header">Ola cab Rides </h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
